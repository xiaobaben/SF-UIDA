from model.model import FeatureExtractor_DSN, SleepMLPDSN, DeepSleepNetEncode
...
def ssc_pretrain_deep(train_dl, val_dl, args):
    """
...
def step1_train(train_dl, val_dl, args):
    ...
    feature_extractor = FeatureExtractor_DSN(args).float().to(device)
    sleep_classifier = SleepMLPDSN(args).float().to(device)
    feature_encoder = DeepSleepNetEncode().float().to(device)

from model.model import FeatureExtractorRec, RecSleepNetEncode, SleepMLPRec
...
def ssc_pretrain_rec(train_dl, val_dl, args):
    """
...
def step1_train(train_dl, val_dl, args):
    ...
    feature_extractor = FeatureExtractorRec(args).float().to(device)
    sleep_classifier = SleepMLPRec(args).float().to(device)
    feature_encoder = RecSleepNetEncode().float().to(device)
    ...
    for epoch in range(1, args["epoch"]+1):
        ...
        for batch_idx, data in enumerate(train_dl):
            ...
            # EEG + EOG
            eeg_eog_feature, ori_x, rec_x = feature_extractor(eeg, eog)  # batch, 20, 512
            ...
            rec_loss = rec_criterion(ori_x, rec_x)
            loss_classifier = classifier_criterion(pred, label.long())

            total_loss = loss_classifier + 0.00001 * rec_loss
...
def step1_dev(model, val_dl, args, model_param):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_dl):
            ...
            eeg_eog_feature, _, _ = model[0](eeg, eog)
